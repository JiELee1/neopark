...
class TicketServiceTest {
	...
	@BeforeEach
	void init() {
		user = createUser(SocialType.GOOGLE, "socialId", "nickname", UserRole.ROLE_ADMIN, false, LocalDate.now(),
			"email@naver.com");
		stadium = createStadium("name", "address", "imageUrl");
		musical = createMusical("title", "description", LocalDate.now(), LocalDate.now().plusDays(5), "thumbnailUrl",
			ViewRating.ALL, Genre.COMEDY, 60, user, stadium);
	...
	void createTicketIsOverFail() {
		...
		Schedule pastSchedule = createSchedule(LocalDateTime.now().minusHours(10), LocalDateTime.now().minusHours(8),
			musical);
	...
	void isExistTicketByMusical() {
		...
		class DeleteTicket {
			...
			void deleteTicketSuccess() {
				...
				when(scheduleSeatRepository.findByScheduleAndSeat(schedule, seat)).thenReturn(
					Optional.of(savedScheduleSeat));
